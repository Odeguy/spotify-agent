...
const searchSpotify = {
    ...
    schema: {
        ...
        type: z
            ...
            .describe('The type of item to search for either track, album, artist, or playlist'),
        limit: z
            .number()
            .min(1)
            .max(50)
    ...
    handler: async (args, _extra) => {
        ...
        try {
            const results = await handleSpotifyRequest(async (spotifyApi) => {
                return await spotifyApi.search(query, [type], undefined, limitValue);
            });
            ...
            if (type === 'track' && results.tracks) {
                ...
                formattedResults = results.playlists.items
                    .map((playlist, i) => {
                    return `${i + 1}. "${playlist?.name ?? 'Unknown Playlist'} (${playlist?.description ?? 'No description'} tracks)" by ${playlist?.owner?.display_name} - ID: ${playlist?.id}`;
                })
                    .join('\n');
            }
            return {
...
const getPlaylistTracks = {
    ...
    handler: async (args, _extra) => {
        ...
        const playlistTracks = await handleSpotifyRequest(async (spotifyApi) => {
            return await spotifyApi.playlists.getPlaylistItems(playlistId, undefined, undefined, limit, offset);
        });
        ...
        const formattedTracks = playlistTracks.items
            ...
            const { track } = item;
            if (!track)
                return `${offset + i + 1}. [Removed track]`;
            if (isTrack(track)) {
                const artists = track.artists.map((a) => a.name).join(', ');
                const duration = formatDuration(track.duration_ms);
...
const getUsersSavedTracks = {
    ...
    handler: async (args, _extra) => {
        ...
        const savedTracks = await handleSpotifyRequest(async (spotifyApi) => {
            return await spotifyApi.currentUser.tracks.savedTracks(limit, offset);
        });
...
const getQueue = {
    name: 'getQueue',
    description: 'Get a list of the currently playing track and the next items in your Spotify queue',
    schema: {
        limit: z
            .number()
            .min(1)
        ...
        try {
            ...
            if (current) {
                ...
                const artists = Array.isArray(current?.artists)
                    ? current.artists
                        ...
                        .join(', ')
                    : 'Unknown';
                const duration = typeof current?.duration_ms === 'number'
            ...
            const formatted = toShow
                ...
                const artists = Array.isArray(track?.artists)
                    ? track.artists
                        ...
                        .join(', ')
                    : 'Unknown';
                const duration = typeof track?.duration_ms === 'number'
            ...
            return {
                content: [
                    {
                        type: 'text',
                        text: `Error fetching queue: ${error instanceof Error ? error.message : String(error)}`,
                    },
                ],
            };
        }
